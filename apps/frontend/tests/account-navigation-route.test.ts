...
import { GET } from "../src/app/api/account/navigation/route";

const originalDiscoveryApiBaseUrl = process.env.DISCOVERY_API_BASE_URL;

describe("account navigation route", () => {
  beforeEach(() => {
  ...
  it("proxies GET navigation requests", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ account_route: "/settings" }), {
        status: 200,
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      "http://backend:8080/api/account/navigation",
  ...
  it("returns 502 when configuration is missing", async () => {
    ...
    const response = await GET(new NextRequest("http://localhost/api/account/navigation"));
