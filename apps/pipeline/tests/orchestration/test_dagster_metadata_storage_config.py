...
def test_compose_dagit_healthcheck_validates_workspace_load() -> None:
    """Dagit health should include workspace GraphQL validation in compose."""
    compose_yaml = Path("docker-compose.yml").read_text(encoding="utf-8")

    assert "http://localhost:3000/graphql" in compose_yaml
    assert "workspaceOrError" in compose_yaml
    assert "locationEntries" in compose_yaml
