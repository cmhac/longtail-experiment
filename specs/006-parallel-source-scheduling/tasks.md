...
- [ ] T001 Add orchestration test target filters for cadence and parallel execution in apps/pipeline/project.json
- [ ] T002 Add local verification command notes for feature 006 in specs/006-parallel-source-scheduling/quickstart.md
- [ ] T003 [P] Add placeholder architecture note for per-source scheduling in docs/architecture/monorepo-boundaries.md
- [ ] T004 [P] Add onboarding note for source schedule metadata maintenance in docs/onboarding/monorepo-baseline.md
- [ ] T005 [P] Add local runbook notes for bounded parallel ingestion validation in docs/runbooks/local-stack-baseline.md
...
- [ ] T006 Add source schedule policy and eligibility snapshot ORM models in libs/db/src/db/models/ingestion_runtime.py
- [ ] T007 Export new runtime models in libs/db/src/db/models/**init**.py
- [ ] T008 Create migration for schedule policy and eligibility persistence in libs/db/alembic/versions/0003_source_schedule_and_eligibility.py
...
- [ ] T016 Add foundational unit tests for schedule policy validation and due selection in apps/pipeline/tests/orchestration/test_source_schedule_policy.py
- [ ] T017 Add migration/model tests for new runtime entities in libs/db/tests/test_ingestion_runtime_models.py
...
- [ ] T038 [P] [US3] Add source eligibility and outcome audit contract test in apps/pipeline/tests/orchestration/test_run_visibility_audit.py
- [ ] T039 [P] [US3] Add operator triage query test for deferred/not-due reason codes in apps/backend/tests/contract/test_ingest_audit_query_contract.py
- [ ] T040 [P] [US3] Add run-level aggregate consistency test (sum of source states equals run counters) in apps/pipeline/tests/orchestration/test_run_visibility_audit.py
...
- [ ] T046 [P] Update feature-specific commands and outcomes in specs/006-parallel-source-scheduling/quickstart.md
