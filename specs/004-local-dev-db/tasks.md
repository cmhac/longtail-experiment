...
- [ ] T001 Add local DB environment variables to docker/compose/stack.env
- [ ] T002 Add development PostgreSQL service scaffold with persistent volume wiring in docker-compose.yml
- [ ] T003 [P] Add Alembic configuration file for shared DB migrations in libs/db/alembic.ini
- [ ] T004 [P] Create local migration runner script in tools/quality/local-stack/run-db-migrations.sh
- [ ] T005 [P] Create migration revision status script in tools/quality/local-stack/check-db-revision.sh
- [ ] T006 [P] Add executable permissions and shell strict-mode headers for new local-stack scripts in tools/quality/local-stack/run-db-migrations.sh and tools/quality/local-stack/check-db-revision.sh
...
- [ ] T007 Configure shared DB URL resolution for Alembic runtime in libs/db/alembic/env.py
- [ ] T008 [P] Add DB connection settings helper for local migration/runtime commands in libs/db/src/db/settings.py
- [ ] T009 [P] Add fail-fast migration wrapper behavior and recovery messaging in tools/quality/local-stack/run-db-migrations.sh
- [ ] T010 [P] Add deterministic revision baseline assertion logic in tools/quality/local-stack/check-db-revision.sh
- [ ] T011 Add local DB service health checks to stack verification flow in tools/quality/local-stack/test-compose-stack.sh
- [ ] T012 [P] Add foundational migration readiness smoke test for config and command contracts in libs/db/tests/test_migration_readiness.py
- [ ] T013 [P] Add foundational repeatable rerun safety test for migration wrapper scripts in libs/db/tests/test_migration_rerun_contract.py
- [ ] T014 Add local defect evidence template for implementation tracking in specs/004-local-dev-db/defect-log.md
- [ ] T015 Validate foundational quality gates for shared DB and local-stack tooling in libs/db/tests/test_migration_readiness.py and libs/db/tests/test_migration_rerun_contract.py
...
- [ ] T016 [P] [US1] Add compose service presence and health test coverage in apps/backend/tests/test_local_db_bootstrap.py
- [ ] T017 [P] [US1] Add environment default/value validation test for local DB profile in apps/pipeline/tests/test_local_db_profile_defaults.py
- [ ] T018 [P] [US1] Add integration shell test for bootstrap success path in tools/quality/local-stack/test-local-db-bootstrap.sh

### Implementation for User Story 1

- [ ] T019 [US1] Implement full PostgreSQL local service definition with named volume and healthcheck in docker-compose.yml
- [ ] T020 [US1] Define canonical local DB defaults (host/port/db/user/password source) in docker/compose/stack.env
- [ ] T021 [US1] Extend stack verification to fail when local DB service is missing/unhealthy in tools/quality/local-stack/test-compose-stack.sh
- [ ] T022 [US1] Add persistent-by-default behavior and explicit reset-only wording to setup runbook in docs/runbooks/local-stack-baseline.md
- [ ] T023 [US1] Add development-only warning language for local DB usage in docs/onboarding/monorepo-baseline.md
- [ ] T024 [US1] Capture US1 bootstrap validation evidence and timing notes in specs/004-local-dev-db/research.md
- [ ] T025 [US1] Verify US1 quality gates and coverage in apps/backend/tests/test_local_db_bootstrap.py and apps/pipeline/tests/test_local_db_profile_defaults.py
...
- [ ] T026 [P] [US2] Add migration fail-fast contract tests for first-error stop behavior in libs/db/tests/test_migration_fail_fast.py
- [ ] T027 [P] [US2] Add migration revision status verification tests in libs/db/tests/test_migration_revision_status.py
- [ ] T028 [P] [US2] Add backend-facing migration command smoke tests in apps/backend/tests/test_local_db_migration_commands.py

### Implementation for User Story 2

- [ ] T029 [US2] Implement canonical Alembic local configuration with shared migration script location in libs/db/alembic.ini
- [ ] T030 [US2] Update Alembic environment to use local DB URL and metadata safely for online/offline flows in libs/db/alembic/env.py
...
- [ ] T037 [P] [US3] Add backend regression tests for reproduced local setup defects in apps/backend/tests/test_local_db_defect_regressions.py
- [ ] T038 [P] [US3] Add pipeline regression tests for reproduced local setup defects in apps/pipeline/tests/test_local_db_defect_regressions.py
- [ ] T039 [P] [US3] Add shared DB repeatable rerun regression test coverage in libs/db/tests/test_local_db_repeatability.py

### Implementation for User Story 3

- [ ] T040 [US3] Track each reproducible defect with symptom, root cause, and fix summary in specs/004-local-dev-db/defect-log.md
- [ ] T041 [US3] Implement fixes for all reproduced compose bootstrap defects in docker-compose.yml and docker/compose/stack.env
- [ ] T042 [US3] Implement fixes for all reproduced migration flow defects in tools/quality/local-stack/run-db-migrations.sh and tools/quality/local-stack/check-db-revision.sh
- [ ] T043 [US3] Update defect recovery guidance for each resolved issue in docs/runbooks/local-stack-baseline.md
- [ ] T044 [US3] Capture per-defect verification command evidence in specs/004-local-dev-db/research.md
- [ ] T045 [US3] Verify US3 quality gates and coverage in apps/backend/tests/test_local_db_defect_regressions.py and apps/pipeline/tests/test_local_db_defect_regressions.py
...
- [ ] T046 [P] Align architecture boundary documentation for local DB responsibilities in docs/architecture/monorepo-boundaries.md
- [ ] T047 [P] Update AGENTS.md with canonical local DB and migration readiness commands in AGENTS.md
- [ ] T048 [P] Add local DB readiness verification helper script combining bootstrap, migrate, and revision checks in tools/quality/local-stack/test-db-readiness.sh
- [ ] T049 Run quickstart validation flow and record pass/fail evidence in specs/004-local-dev-db/research.md
- [ ] T050 Run full affected quality suite and local stack verification via package.json scripts and tools/quality/local-stack/test-compose-stack.sh
- [ ] T051 [P] Verify shell script portability and strict error behavior across local-stack scripts in tools/quality/local-stack/run-db-migrations.sh and tools/quality/local-stack/check-db-revision.sh
- [ ] T052 [P] Update onboarding and runbook command matrices to include new readiness script in docs/onboarding/monorepo-baseline.md and docs/runbooks/local-stack-baseline.md
