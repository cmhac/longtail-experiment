"""Postgres repository implementation for auth/account persistence."""
...
from .interfaces import AuthManagementRepository
...
class PostgresAuthManagementRepository(AuthManagementRepository):
    """Persist auth/account entities in PostgreSQL."""
    ...
    def _serialize_user_row(row: dict[str, object]) -> dict[str, object]:
        ...
        lockout_until = (
            lockout_until_value if isinstance(lockout_until_value, datetime) else None
        )
        ...
        return {
            ...
            "account_status": str(row["account_status"]),
            "failed_sign_in_count": (
                int(failed_sign_in_value)
                if isinstance(failed_sign_in_value, int | str)
                else 0
            ),
