...
dagster_db_state=$(printf '%s\n' "$dagster_db_ps_json" | grep -c '"Service":"dagster_db"' || true)
db_healthy=0
...
for _ in $(seq 1 "$health_wait_seconds"); do
  ...
  if [[ "$db_healthy" -gt 0 && "$dagster_db_healthy" -gt 0 && "$dagit_healthy" -gt 0 ]]; then
  ...
  if (( elapsed_seconds == 1 || elapsed_seconds % 15 == 0 )); then
    ...
    echo "Waiting for health: elapsed=${elapsed_seconds}s/${health_wait_seconds}s db_healthy=${db_healthy} dagster_db_healthy=${dagster_db_healthy} dagit_healthy=${dagit_healthy} dagit_status=${dagit_status}"
  fi
  last_report_second="$elapsed_seconds"
  sleep 1
done
if [[ "$pipeline_state" -eq 0 || "$backend_state" -eq 0 || "$frontend_state" -eq 0 || "$dagit_state" -eq 0 || "$db_state" -eq 0 || "$dagster_db_state" -eq 0 ]]; then
  echo "Pipeline, backend, frontend, dagit, db, or dagster_db service is missing." >&2
...
if [[ "$dagster_db_healthy" -eq 0 ]]; then
  echo "Dagster metadata DB service is not healthy." >&2
